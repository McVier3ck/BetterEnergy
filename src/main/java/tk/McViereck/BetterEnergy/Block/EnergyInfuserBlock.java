...
public class EnergyInfuserBlock extends BlockContainer {
	...
	public void registerBlockIcons(IIconRegister reg) {
				...
				this.blockIcon = reg.registerIcon("betterenergy:MachineBlock_Side");
		        this.frontIcon = reg.registerIcon("betterenergy:EnergyInfuser");
		        this.frontIcon_active = reg.registerIcon("betterenergy:EnergyInfuser_Work");
		        
	}
	
	@Override
	public IIcon getIcon(int side, int meta) {
		if (side ==2 && meta == 6) return frontIcon_active; 
		if (side ==3 && meta == 7) return frontIcon_active; 
		if (side ==4 && meta == 8) return frontIcon_active; 
		if (side ==5 && meta == 9) return frontIcon_active; 
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitx, float gity, float hitz) {
		...
		if(world.isRemote) {
			...
			if(player.inventory.getCurrentItem() == null || !(player.inventory.getCurrentItem().getItem() instanceof IToolHammer)) {
				FMLNetworkHandler.openGui(player, BetterEnergy.instance, BetterEnergy.EnergyInfuserID, world, x, y, z);
				return true;
			}
		...
		if(player.inventory.getCurrentItem() != null) {
			if(player.inventory.getCurrentItem().getItem() instanceof IToolHammer) {
				if(!player.isSneaking()) {
					int meta = world.getBlockMetadata(x, y, z);
					if(meta == 2) world.setBlockMetadataWithNotify(x, y, z, 5, 2);
					if(meta == 3) world.setBlockMetadataWithNotify(x, y, z, 4, 2);
					if(meta == 4) world.setBlockMetadataWithNotify(x, y, z, 2, 2);
					if(meta == 5) world.setBlockMetadataWithNotify(x, y, z, 3, 2);
					
					if(meta == 6) world.setBlockMetadataWithNotify(x, y, z, 9, 2);
					if(meta == 7) world.setBlockMetadataWithNotify(x, y, z, 8, 2);
					if(meta == 8) world.setBlockMetadataWithNotify(x, y, z, 6, 2);
					if(meta == 9) world.setBlockMetadataWithNotify(x, y, z, 7, 2);
				} else {
	...
	public TileEntity createNewTileEntity(World world, int side) {
		...
		return new TileEntityEnergyInfuser();
	}
